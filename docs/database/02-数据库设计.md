...
- **建表机制（2026-09-24 说明）**: 运行时表由 Hibernate `ddl-auto: update` 按 JPA 实体自动创建，本目录 `01-schema.sql` 为完整参考 DDL 而非实际建库来源；**12 张表有 Repository 读写**（2026-09-24 更新：前述 8 张 + trace 两张 + t_intent_node + t_query_term_mapping，后四张随 trace/意图树/词映射落地激活），其余 11 张为"有实体无读写"或"仅设计未接线"的表，逐张状态见 [known-gaps](../实现差距清单.md#database)。
...
| 表 | 用途（规划） | 状态 |
|----|------|------|
| `t_chunk_hierarchy` | Parent/Leaf 父子关系 + window_content 句窗内容，服务 Sentence Window + Auto-Merging | 🚫 零引用（SWA 实际尝试走 Milvus/ES metadata 且已失效，见 [known-gaps](../实现差距清单.md#swa)） |
