...
- **想了解系统能做什么** → 根 [README.md](../README.md) → [02-core-components](architecture/02-core-components.md)
- **想了解系统"实际"是什么样的（含缺陷）** → [known-gaps.md](./known-gaps.md) ⭐ 对账基准
- **想调接口** → [api-reference](./api-reference.md)（全端点 + 流式协议）
- **想改配置** → [configuration-reference](./configuration-reference.md)（全配置键）
- **想跑起来** → [01-quick-start](guides/01-quick-start.md)；**日常运维/排障** → [03-operations](guides/03-operations.md)
...
├── api-reference.md                       # 全量 REST 接口参考（19 Controller / 60+ 端点 / NDJSON 协议）
├── configuration-reference.md             # 配置参考全集（config.yaml + application.yml + react 段）
...
│   ├── 04-frontend.md                     # 前端架构（路由/store/流式解析/引用面板/构建）
│   ├── 05-chunking-strategies.md          # 五种分块策略详解（算法/参数/决策树/SWA 父子结构）
│   └── 06-multimodal.md                   # 多模态能力（视觉对话/文档 OCR 增强/实验结论）
│
├── database/                              # 数据库文档
...
│   ├── 02-deployment.md                   # 部署文档（水平扩容限制/性能基线）
│   └── 03-operations.md                   # 运维手册（test-agent/watchdog/stress-test/告警锚点/排障 runbook）
│
...
| 文档 | 状态 | 说明 |
|------|------|------|
| [known-gaps](./known-gaps.md) | ✅ 现状 | 实现差距清单（含修复状态与优先级），对账基准 |
| [api-reference](./api-reference.md) | ✅ 现状 | 全量接口：19 Controller / 认证与权限约定 / 流式 NDJSON 协议 / 词映射·意图树·traces 新 API 契约 |
| [configuration-reference](./configuration-reference.md) | ✅ 现状 | 全配置键（config.yaml / application.yml / react 段）+ 配置常见坑 |
...
| [04-frontend](architecture/04-frontend.md) | ✅ 现状文档 | 前端架构：路由树/三 store/NDJSON 流式解析/引用面板/与后端契约对齐点 |
| [05-chunking-strategies](architecture/05-chunking-strategies.md) | ✅ 现状文档 | 五种分块策略：算法细节/参数语义/intelligent 决策树/SWA 父子结构与数据通路 |
| [06-multimodal](architecture/06-multimodal.md) | ✅ 现状文档 | 多模态：视觉对话端点/文档 OCR 增强链路/实验结论与当前边界 |
...
| 文档 | 状态 | 说明 |
|------|------|------|
| [01-quick-start](guides/01-quick-start.md) | ✅ 现状 | 端口/config 机制/Ollama/认证流程均与代码核对，可执行 |
| [02-deployment](guides/02-deployment.md) | ✅ 现状 | 引用仓库真实 compose，含水平扩容限制与性能基线 |
| [03-operations](guides/03-operations.md) | ✅ 现状 | 工具链（test-agent/watchdog/stress-test 入口）、Grafana 面板目录、三个告警锚点、四类故障排障 runbook |

### 评测与测试（testing/）

> ⚠️ **基线时效提示**：下列实测数字测于 2026-09-24 代码修复**之前**——SWA 修复/rerank 混排修复/词映射接入/上下文预算均可能改变结果，重大变更后建议重跑基线（见 [运维手册第四节](guides/03-operations.md)）。
