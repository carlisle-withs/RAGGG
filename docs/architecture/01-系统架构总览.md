...
> 工程缺陷与修复状态的对账基准见 [known-gaps](../实现差距清单.md)；本文与其冲突时以 known-gaps 为准。
...
- **COMPLEX**：`ReActEngine.execute()`（详见 [03-react-engine](./03-ReAct推理引擎.md)）——正常完成聚合检索命中为 sources；**降级时回退真实 RAG 链路重新作答**
...
**水平扩容限制**（详见 [部署文档](../guides/02-部署指南.md)）：Kafka 消费者组天然多实例；会话写锁与 ReAct 缓存为 JVM 级；Milvus 维度/字段不一致会互相触发重建；流式连接需 sticky session。
