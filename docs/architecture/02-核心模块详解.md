...
> 总览见 [01-architecture-overview](./01-系统架构总览.md)；缺陷对账见 [known-gaps](../实现差距清单.md)。
...
[testing/kafka-pipeline-throughput-report-2026-09-21](../testing/Kafka流水线吞吐报告-2026-09-21.md)：16.5 docs/min = 890 chunks/min，零失败；瓶颈下钻定位 **95.1% 耗时在本地 Ollama embedding 单流**（Parse 57ms / Chunk 55ms / 三库持久化 60ms 均可忽略）；扩容路径：topic 扩 3 分区（理论 ~3×）或换 SiliconFlow embedding（5-20×）。
...
| 语料 | 结论 | 报告 |
|---|---|---|
| PubMedQA（英文医学，n=196） | hybrid ≈ vector（Hit@10 100% 召回天花板）；rerank 无增益（当时 key 失效静默回退） | [retrieval-eval-pubmedqa](../testing/检索评测-PubMedQA-2026-09-21.md) |
| CRUD-RAG（中文新闻，n=2394×3） | **CrossEncoder 精排真实增益**（Hit@3 +2.6pp，代价 +0.5s）；纯 RRF 在中文新闻不占优（hybrid vs vector Hit@5 -0.9pp） | [crud-rag-benchmark](../testing/CRUD-RAG中文基准-2026-09-22.md) |
...
完整设计与实现对照见 [03-react-engine](./03-ReAct推理引擎.md)。一句话：**复杂度路由 + 思考→行动→观察循环 + 四重护栏（SQL 只读/循环检测/缓存 TTL/降级回退）**，`react.enabled` 默认关闭、当前 config.yaml 开启。
...
- 实测：综合 0.785（n=20），Context Recall 0.982 最强 / Context Precision 0.640 最弱（结构性原因：PubMedQA 摘要切 4-5 块，top-5 必混邻块），见 [ragas-eval](../testing/RAGAS评测-2026-09-22.md)
