...
export const batchUploadDocuments = async (
  ...
  if (options?.processMode) {
    formData.append("processMode", options.processMode);
  }
  if (options?.chunkStrategy) {
    formData.append("chunkStrategy", options.chunkStrategy);
  }
  if (options?.chunkConfig) {
    formData.append("chunkConfig", options.chunkConfig);
  }
  return api.post<BatchUploadResult, BatchUploadResult>(
    `/knowledge-base/${kbId}/docs/batch-upload`,
    formData,
    {
      headers: {
        "Content-Type": "multipart/form-data"
