...
function BatchUploadDialog({ open, onOpenChange, onSuccess }: BatchUploadDialogProps) {
  ...
  const [selectedStrategy, setSelectedStrategy] = useState("intelligent");

  useEffect(() => {
    if (open) {
      setFiles([]);
      setSelectedStrategy("intelligent");
      getChunkStrategies().then(setStrategies).catch(() => {});
    }
  }, [open]);

  // 自动用策略默认值填充 chunkConfig
  const selectedStrategyConfig = strategies.find((s) => s.value === selectedStrategy)?.defaultConfig || {};
  ...
  const handleSubmit = async () => {
    ...
    try {
      const chunkConfig =
        Object.keys(selectedStrategyConfig).length > 0
          ? JSON.stringify(selectedStrategyConfig)
          : undefined;
      await batchUploadDocuments(kbId, files, {
        chunkStrategy: selectedStrategy,
        chunkConfig
      });
...
<DialogFooter>
          ...
          <Button onClick={handleSubmit} disabled={saving || files.length === 0}>
            {saving ? `上传中 (${files.length})...` : `上传 ${files.length > 0 ? `(${files.length})` : ""}`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
