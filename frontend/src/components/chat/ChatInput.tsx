...
import { Brain, Database, Lightbulb, Send, Square } from "lucide-react";
...
export function ChatInput() {
  ...
  const [kbOptions, setKbOptions] = React.useState<{ id: string; name: string }[]>([]);
  ...
  React.useEffect(() => {
    getKnowledgeBases().then(data => {
      if (data.records && data.records.length > 0) {
        setKbOptions(data.records.map((kb: any) => ({ id: String(kb.id), name: kb.name })));
        setActiveKbId(String(data.records[0].id));
      }
    }).catch(() => {});
  }, []);
        ...
        <div className="relative mt-2 flex items-center gap-2">
          {kbOptions.length > 0 ? (
            <div className="absolute left-0 flex items-center gap-1.5">
              <Database className="h-3.5 w-3.5 text-[#999999]" />
              <select
                value={activeKbId}
                onChange={(e) => setActiveKbId(e.target.value)}
                disabled={isStreaming}
                className="rounded-md border border-[#E5E5E5] bg-white px-2 py-1.5 text-xs text-[#666666] focus:border-[#3B82F6] focus:outline-none"
              >
                {kbOptions.map(kb => (
          ...
          <button
            ...
            className={cn(
              "ml-auto rounded-lg border px-3 py-1.5 text-xs font-medium transition-all",
              deepThinkingEnabled
