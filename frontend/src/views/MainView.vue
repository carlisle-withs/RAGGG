  ...
  <div class="main-layout">
    ...
    <aside class="sidebar" :class="{ collapsed: sidebarCollapsed }">
      ...
      <div class="sidebar-footer">
        <div v-if="authStore.isAuthenticated" class="user-info" :class="{ collapsed: sidebarCollapsed }">
        ...
        <button v-if="authStore.isAdmin" class="nav-item" @click="goToAdmin">
    ...
    <main class="main-content">
      ...
      <header class="top-bar">
        <div class="current-mode">智能问答</div>
      ...
      <div class="chat-container" ref="chatContainer">
        ...
        <div v-if="messages.length === 0" class="welcome">
          ...
          <h1>你好，我是 RAG 助手</h1>
          <p>输入问题进行智能问答，或输入 /search 进行文档检索</p>
      ...
      <div class="input-area">
        <div class="input-wrapper">
          <textarea v-model="inputMessage" class="message-input"
                    placeholder="输入消息，/search 开头进行检索..."
                    @keydown.enter.exact.prevent="handleSend"
                    @input="autoResize"
                    rows="1"></textarea>
          <button class="send-btn" @click="handleSend" :disabled="!inputMessage.trim() || loading">
            ➤
          </button>
        </div>
      </div>
...
const handleSend = async () => {
  ...
  const userMessage = isSearch ? text.slice(8) : text
  const displayMessage = isSearch ? '/search ' + userMessage : text
  ...
  try {
    if (isSearch) {
      const response = await retrieve({ query: userMessage })
